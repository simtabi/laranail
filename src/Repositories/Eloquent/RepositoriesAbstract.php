...
use Simtabi\Laranail\Repositories\Interfaces\RepositoryInterface;
...
abstract class RepositoriesAbstract implements RepositoryInterface
{
    ...
    public function applyBeforeExecuteQuery($data, $isSingle = false)
    {
        if (is_in_admin()) {
            if (!$isSingle) {
                $data = apply_filters('before_get_admin_list_item', $data, $this->originalModel);
            }
        } elseif (!$isSingle) {
            $data = apply_filters('before_get_front_page_item', $data, $this->originalModel);
        } else {
            $data = apply_filters('before_get_home_page_data_single', $data, $this->originalModel);
        }
